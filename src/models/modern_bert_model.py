"""
SB IntuitionsのModernBERT-Ja-130Mを使用したModernBERTモデル
"""
...
class ModernBERTModel(nn.Module):
    """SB IntuitionsのModernBERT-Ja-130Mを使用したモデル"""

    def __init__(
        self, model_name: str = "sbintuitions/modernbert-ja-130m", num_choices: int = 5
    ):
        super().__init__()
        self.model_name = model_name
        self.num_choices = num_choices

        # ModernBERTモデルとトークナイザーを読み込み
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # ModernBERTではpooler_outputがない場合があるので、last_hidden_stateから取得
        if hasattr(outputs, "pooler_output") and outputs.pooler_output is not None:
            ...
            pooled_output = outputs.last_hidden_state[
                :, 0, :
            ]  # (batch_size * num_choices, hidden_size)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        # ModernBERTの設定項目をチェックして追加
        if hasattr(self.bert.config, "num_hidden_layers"):
            model_info["num_layers"] = self.bert.config.num_hidden_layers
        if hasattr(self.bert.config, "num_attention_heads"):
            model_info["num_attention_heads"] = self.bert.config.num_attention_heads
        if hasattr(self.bert.config, "vocab_size"):
            model_info["vocab_size"] = self.bert.config.vocab_size
